package Hometask7.parser;

import Hometask7.exceptions.IllegalCharacterException;
...
public class ExpressionParser {
    ...
    private void initParser(String string) throws ParserException {
        expression = string;
        position = 0;
    ...
    public Expression parseTerm(String string) throws ParserException {
        initParser(string);
        return readTerm();
    }
    ...
    public Expression parseAsPlaceHolder(String string) throws ParserException {
        isPlaceHolder = true;
        return makeParsing(string);
    }
    ...
    private Expression readMultiplier() throws ParserException {
        ...
        if (currentToken == Token.LOWER_LETTER) {
            ...
            if (currentToken != Token.OPEN) {
                //It's var, because we have not encountered an open bracket
    ...
    private Expression readUnary() throws ParserException {
        ...
        if (currentToken == Token.NOT) {
            ...
            Expression unary = readUnary();
            result = new ForAll(variable, unary);
        } else if (currentToken == Token.EXISTS) {
            currentToken = nextToken();
            Expression variable = readVariable();
            Expression unary = readUnary();
            result = new Exists(variable, unary);
        } else if (isPredicate()) {
